 ...
 *
 * The SDK is not a dependency of this app — every version is fetched at runtime from a
 * CDN — so the package is downloaded here instead of read from node_modules. Which
 * version this snapshot is built from lives in scripts/sdk-versions.json:
 *
 *   npm run gen:rest-catalog              # the pinned version
 *   npm run gen:rest-catalog -- 2.27.1    # and update the pin to it
 *
...
const PINS_FILE = resolve(root, 'scripts/sdk-versions.json')
const OUT = resolve(root, 'src/studio/rest/catalog.ts')

const pins = readPins()
const spec = process.argv[2] ?? pins[REST_PKG]
const pkg = await fetchSdkPackage(REST_PKG, spec)
const DIST = resolve(pkg.root, 'dist')

if (process.argv[2]) {
  writeFileSync(PINS_FILE, JSON.stringify({ ...pins, [REST_PKG]: spec }, null, 2) + '\n')
}
...
for (const line of js) {
  ...
  let m = line.match(/^ {2}(?:async\s+)?([a-z]\w*)\([^)]*\)\s*\{$/)
  if (m) {
    curName = m[1]
    curPath = null
    continue
...
const banner = `/* AUTO-GENERATED by scripts/gen-rest-catalog.mjs — do not edit by hand.
   Source: @thoughtspot/rest-api-sdk@${pkg.version} as published (dist/index.d.ts + dist/index.js).
   To pick up new endpoints: \`npm run gen:rest-catalog -- <version>\`, which repins it. */`
...
/**
 * categorical-palette — 24 hues that only have to be distinguishable from each
 * other. Radiant offers six accent colours, which cannot keep these apart, and
 * collapsing them would destroy the thing the colours exist for.
 */
...
export function restGroupColor(group: string): string {
  return REST_GROUP_COLOR[group] ?? REST_GROUP_COLOR.Other
}
